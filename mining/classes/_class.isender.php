...
class isender{
	...
	function RecoveryPassword($user, $pass, $mail){
		...
		$text.= "Login data for user account: <BR />";
		$text.= "<b>Login:</b> {$user}<BR />";
		$text.= "<b>Password:</b> {$pass}<BR />";
		$text.= "Link to enter the office: <a href='http://".$this->Hosts."/signin'>http://".$this->Hosts."/signin</a>";
		$subject = "Recover a forgotten password in the system \"".$this->Hosts."\"";
