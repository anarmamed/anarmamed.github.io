  ...
  { 
    ...
    {    
      ...
      {  
        ...
        $pay_user = number_format($price - ($price*(2.5/10)), 2); //оплата пользователю
        
        //зачисление денег за просмотр пользователю
        $db->query("UPDATE db_users_b SET `money_p` = `money_p` + '".$pay_user."', `serf_view` = `serf_view` + '1'	WHERE id = '".$_SESSION['user_id']."'");
    ...
    {
      exit('<div class="blockerror">ERROR!<br /><span>НЕВЕРНО РЕШЕНА ЗАДАЧА</span></div>');
    } 
  }
  else if ($num == 0)
  { 
